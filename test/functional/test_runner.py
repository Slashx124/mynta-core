...
def main():
    ...
    # Check that the build was configured with wallet, utils, and myntad
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_myntad = config["components"].getboolean("ENABLE_MYNTAD")
    if not (enable_wallet and enable_cli and enable_myntad):
        print("No functional tests to run. Wallet, utils, and myntad must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if myntad is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "myntad"]) is not None:
            print("%sWARNING!%s There is already a myntad process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "MYNTAD" not in os.environ:
        os.environ["MYNTAD"] = build_dir + '/src/myntad' + exeext
        os.environ["MYNTACLI"] = build_dir + '/src/mynta-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `mynta-cli help` (`rpc_interface.txt`).
