...
dnl MYNTA_QT_PATH_PROGS([FOO], [foo foo2], [/path/to/search/first], [continue if missing])
...
AC_DEFUN([MYNTA_QT_PATH_PROGS],[
  MYNTA_QT_CHECK([
    ...
    if test "x$$1" = x && test "x$4" != xyes; then
      MYNTA_QT_FAIL([$1 not found])
    fi
  ])
])

dnl Initialize qt input.
dnl This must be called before any other MYNTA_QT* macros to ensure that
dnl input variables are set correctly.
dnl CAUTION: Do not use this inside of a conditional.
AC_DEFUN([MYNTA_QT_INIT],[
...
dnl
dnl   MYNTA_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _MYNTA_QT_FIND_LIBS
dnl Outputs: Sets variables for all qt-related tools.
dnl Outputs: raven_enable_qt, raven_enable_qt_dbus, raven_enable_qt_test
AC_DEFUN([MYNTA_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  MYNTA_QT_CHECK([_MYNTA_QT_FIND_LIBS])

  dnl This is ugly and complicated. Yuck. Works as follows:
  dnl For Qt5, we can check a header to find out whether Qt is build
  dnl statically. When Qt is built statically, some plugins must be linked into
  dnl the final binary as well.
  dnl _MYNTA_QT_CHECK_STATIC_PLUGIN does a quick link-check and appends the
  ...
  if test "x$raven_cv_static_qt" = xyes; then
    ...
    if test "x$TARGET_OS" != xandroid; then
      _MYNTA_QT_CHECK_STATIC_PLUGIN([QMinimalIntegrationPlugin], [-lqminimal])
    ...
    if test "x$TARGET_OS" = xwindows; then
      ...
      _MYNTA_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      ...
      _MYNTA_QT_CHECK_STATIC_PLUGIN([QXcbIntegrationPlugin], [-lqxcb])
      ...
      _MYNTA_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
      _MYNTA_QT_CHECK_STATIC_PLUGIN([QMacStylePlugin], [-lqmacstyle])
  ...
  MYNTA_QT_PATH_PROGS([MOC], [moc-qt5 moc5 moc], $qt_bin_path)
  MYNTA_QT_PATH_PROGS([UIC], [uic-qt5 uic5 uic], $qt_bin_path)
  MYNTA_QT_PATH_PROGS([RCC], [rcc-qt5 rcc5 rcc], $qt_bin_path)
  MYNTA_QT_PATH_PROGS([LRELEASE], [lrelease-qt5 lrelease5 lrelease], $qt_bin_path)
  MYNTA_QT_PATH_PROGS([LUPDATE], [lupdate-qt5 lupdate5 lupdate],$qt_bin_path, yes)
...
dnl
dnl _MYNTA_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_MYNTA_QT_CHECK_STATIC_PLUGIN], [
...
AC_DEFUN([_MYNTA_QT_FIND_LIBS],[
  MYNTA_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [],
                      [MYNTA_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  MYNTA_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [],
                      [MYNTA_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  MYNTA_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [],
                      [MYNTA_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  MYNTA_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [],
                      [MYNTA_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])
