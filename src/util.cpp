...
// Copyright (c) 2017-2019 The Mynta Core developers
...
const char *const MYNTA_CONF_FILENAME = "mynta.conf";
const char *const MYNTA_PID_FILENAME = "myntad.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Mynta
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Mynta
    // Mac: ~/Library/Application Support/Mynta
    // Unix: ~/.mynta
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mynta";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Mynta";
#else
    // Unix
    return pathRet / ".mynta";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string &confPath)
{
    ...
    if (!streamConfig.good())
        return; // No mynta.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override mynta.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", MYNTA_PID_FILENAME));
...
std::string CopyrightHolders(const std::string &strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Mynta Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Mynta Core") == std::string::npos)
    {
        strCopyrightHolders += "\n" + strPrefix + "The Mynta Core developers";
    }
