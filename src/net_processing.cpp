...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    // ============================================================
    // LLMQ Message Handling
    // ============================================================
    
    else if (strCommand == NetMsgType::QSIGSHARE) {
        // Quorum signature share - relay to signing manager
        if (llmq::signingManager) {
            // Parse the signature share
            uint256 quorumHash;
            uint256 id;
            uint256 proTxHash;
    ...
    else if (strCommand == NetMsgType::ISLOCK) {
        // InstantSend lock
        if (llmq::instantSendManager) {
            llmq::CInstantSendLock islock;
            vRecv >> islock;
            
            CValidationState state;
            if (llmq::instantSendManager->ProcessInstantSendLock(islock, state)) {
                LogPrint(BCLog::NET, "LLMQ: Processed InstantSend lock for %s from peer=%d\n", 
                         islock.txid.ToString().substr(0, 16), pfrom->GetId());
                // Relay to other peers
                connman->ForEachNode([&connman, &islock](CNode* pnode) {
                    if (pnode->fSuccessfullyConnected) {
                        CNetMsgMaker msgMaker(pnode->GetSendVersion());
                        connman->PushMessage(pnode, msgMaker.Make(NetMsgType::ISLOCK, islock));
                    }
                });
            } else {
                LogPrint(BCLog::NET, "LLMQ: Failed to process InstantSend lock from peer=%d: %s\n",
        ...
        // ChainLock signature
        if (llmq::chainLocksManager) {
            llmq::CChainLockSig clsig;
            vRecv >> clsig;
            
            CValidationState state;
            if (llmq::chainLocksManager->ProcessChainLock(clsig, state)) {
                LogPrint(BCLog::NET, "LLMQ: Processed ChainLock at height %d from peer=%d\n", 
                         clsig.nHeight, pfrom->GetId());
                // Relay to other peers
                connman->ForEachNode([&connman, &clsig](CNode* pnode) {
                    if (pnode->fSuccessfullyConnected) {
                        CNetMsgMaker msgMaker(pnode->GetSendVersion());
                        connman->PushMessage(pnode, msgMaker.Make(NetMsgType::CLSIG, clsig));
                    }
                });
            } else {
                LogPrint(BCLog::NET, "LLMQ: Failed to process ChainLock from peer=%d: %s\n",
                         pfrom->GetId(), state.GetRejectReason());
            }
        }
    }
    
    else if (strCommand == NetMsgType::QGETDATA || strCommand == NetMsgType::QDATA) {
        // Quorum data request/response - log but don't process yet
        LogPrint(BCLog::NET, "LLMQ: Received %s from peer=%d (not fully implemented)\n",
