 ...
 * between myntad, and raven-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    FlushWallets();
#endif
    GenerateMyntas(false, 0, GetParams());
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), MYNTA_PID_FILENAME));
#endif
...
/** Sanity checks
 *  Ensure that Mynta is running in a usable environment with all
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", MYNTA_CONF_FILENAME)).string());
    ...
    GenerateMyntas(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
