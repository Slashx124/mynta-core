...
// Special transaction support (DIP2/DIP3)
#include "evo/providertx.h"
#include "evo/specialtx.h"
#include "evo/deterministicmns.h"

// LLMQ support (InstantSend & ChainLocks)
...
#if defined(NDEBUG)
# error "Mynta cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Mynta Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
    ...
    // Handle special transactions (DIP2/DIP3 - deterministic masternodes)
    // Special transactions use nVersion >= 3 with nType != 0
    ...
    // ============================================================
    // InstantSend Conflict Check
    // Reject transactions that conflict with InstantSend-locked inputs
    // ============================================================
    if (llmq::instantSendManager && llmq::instantSendManager->HasConflictingLock(tx)) {
        return state.DoS(10, false, REJECT_CONFLICT, "tx-islock-conflict",
                        false, "Transaction conflicts with InstantSend-locked inputs");
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, CAssetsCache* assetsCache = nullptr, bool fJustCheck = false, bool ignoreAddressIndex = false)
{
    ...
    if (!CheckBlock(block, state, chainparams, !fJustCheck, !fJustCheck)) // Force the check of asset duplicates when connecting the block
...
/**
 * Check block header validity, particularly Proof of Work.
 * 
 * @param block The block header to check
 * @param state Validation state for error reporting
 * @param chainparams Chain parameters (provides consensus params AND checkpoints)
 * @param fCheckPOW Whether to verify proof of work
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, bool fCheckPOW = true)
{
    ...
    // KawPoW optimization: For blocks below checkpoint height, we can verify
    // PoW using just the mix_hash without full KawPoW computation
    ...
    // Verify mix_hash for KawPoW blocks
    if (fCheckPOW && block.nTime >= nKAWPOWActivationTime) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, const CChainParams& chainparams, bool fCheckPOW, bool fCheckMerkleRoot, bool fDBCheck)
{
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // MYNTA LAUNCH: Check chain start time - no blocks before official launch
    // This prevents premature mining while allowing pre-release binary distribution
    if (consensusParams.nChainStartTime > 0 && nHeight > 0) {
        if (block.GetBlockTime() < consensusParams.nChainStartTime) {
            return state.DoS(100, false, REJECT_INVALID, "chain-not-started",
                false, strprintf("block timestamp %d is before chain start time %d (launch: Jan 14, 2026 4PM PST)",
                    block.GetBlockTime(), consensusParams.nChainStartTime));
        }
    }
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, CAssetsCache* assetCache)
{
    ...
    // Verify block doesn't conflict with a ChainLocked chain
    // ============================================================
    if (llmq::chainLocksManager && pindexPrev) {
        // Check if this block would orphan a ChainLocked block
        // If we have a ChainLock at height H, reject any block that 
        // would create a competing chain below H
        int chainLockHeight = llmq::chainLocksManager->GetBestChainLockHeight();
        if (chainLockHeight > 0 && nHeight <= chainLockHeight) {
            // Check if this block matches the ChainLocked block at this height
            if (llmq::chainLocksManager->IsChainLocked(nHeight)) {
                llmq::CChainLockSig bestCL = llmq::chainLocksManager->GetBestChainLock();
                // The best ChainLock must be on the same chain as this block
                // If we're at or below the ChainLock height but have different hash, reject
                if (nHeight == chainLockHeight && block.GetHash() != bestCL.blockHash) {
                    return state.DoS(100, false, REJECT_INVALID, "bad-block-chainlock-conflict",
                                    false, strprintf("block at height %d conflicts with ChainLock", nHeight));
                }
            }
        }
    }
    ...
    // 
    // CRITICAL FOR CONSENSUS: This validation MUST be deterministic across all nodes.
    // The validation uses GetMNPayee(pindexPrev) which:
    // - Retrieves the MN list state at pindexPrev (NOT the tip)
    // - Computes payee using the block hash as entropy
    // - Returns the same result regardless of current tip or reorg state
    //
    // Edge cases handled:
    // 1. Reorg boundary: Uses list state from pindexPrev, not current tip
    // 2. Script changes: Uses scriptPayout from the list at pindexPrev
    // 3. PoSe bans: Only considers masternodes valid at pindexPrev
    // 4. No masternodes: Gracefully skips if list is empty (bootstrap period)
    // 5. Operator reward: Validates primary payout; operator payout is optional
    // 6. Grace period: Payments not enforced immediately after activation
    //
    ...
    if (IsMasternodePaymentEnforced(nHeight) && deterministicMNManager && pindexPrev) {
        ...
        if (expectedPayee) {
            ...
            // The GetMNPayee already filters for valid MNs, but double-check
            if (!expectedPayee->IsValid()) {
                LogPrintf("ContextualCheckBlock: Expected payee %s is not valid (should not happen)\n",
        ...
        // If no valid masternodes, no payment required (early network bootstrap)
    }
...
static bool AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fFromLoad = false)
{
    ...
    if (!CheckBlock(block, state, chainparams, true, true) ||
...
{
    {
        ...
        // belt-and-suspenders.
        bool ret = CheckBlock(*pblock, state, chainparams, true, true);
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel >= 1 && !CheckBlock(block, state, chainparams, fCheckPoW, fCheckMerkleRoot, fDBCheck)) // fCheckAssetDuplicate set to false, because we don't want to fail because the asset exists in our database, when loading blocks from our asset databse
